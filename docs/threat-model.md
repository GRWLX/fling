...
| Certificate issue CLI | `sudo sshfling --certificate -t ...`, `sshfling cert issue --certificate` | Operator/admin -> CA key | Signs public keys with force-command critical option and policy cap after a CA keypair exists. Certificate setup rejects dry-run and missing CA keypairs. | `bin/sshfling::cmd_setup_certificate`, `sign_user_certificate` |
