...
if ! command -v raku >/dev/null 2>&1; then
  if [[ "${EUID:-$(id -u)}" -eq 0 ]]; then
    apt-get update >&2
    apt-get install -y --no-install-recommends raku >&2
  elif command -v sudo >/dev/null 2>&1; then
    sudo apt-get update >&2
    sudo apt-get install -y --no-install-recommends raku >&2
  else
    echo "sudo or root privileges are required to install raku" >&2
    exit 1
  fi
fi
