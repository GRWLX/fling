        ...
        in {
          default = pkgs.stdenvNoCC.mkDerivation {
            ...
            src = pkgs.fetchurl {
              ...
              hash = "sha256-eP64/MA0TJ2j39HvUwXpY329NZdJ3x/NbFHtfulh1MQ=";
            };
            ...
            installPhase = ''
              ...
              substituteInPlace $out/share/sshfling/templates/production/sshfling-session                 --replace-fail                   'session_user_path="''${PATH:-}"'                   'session_user_path="${pkgs.lib.makeBinPath nativeRuntimePath}:''${PATH:-}"'                 --replace-fail                   'PATH="/usr/sbin:/usr/bin:/sbin:/bin:'                   'PATH="${pkgs.lib.makeBinPath nativeRuntimePath}:/usr/sbin:/usr/bin:/sbin:/bin:'
              ...
              wrapProgram $out/libexec/sshfling/sshfling-linux-account                 --set SSHFLING_NATIVE_TOOL_PATH ${pkgs.lib.makeBinPath nativeRuntimePath}
              wrapProgram $out/libexec/sshfling/sshfling-unix-identity                 --set SSHFLING_NATIVE_TOOL_PATH ${pkgs.lib.makeBinPath nativeRuntimePath}
              wrapProgram $out/bin/sshfling                 --prefix PATH : ${pkgs.lib.makeBinPath runtimePath}                 --set SSHFLING_NATIVE_TOOL_PATH ${pkgs.lib.makeBinPath nativeRuntimePath}                 --set SSHFLING_LINUX_ACCOUNT_HELPER $out/libexec/sshfling/sshfling-linux-account                 --set SSHFLING_UNIX_IDENTITY_HELPER $out/libexec/sshfling/sshfling-unix-identity
              runHook postInstall
            '';
            meta = with pkgs.lib; {
